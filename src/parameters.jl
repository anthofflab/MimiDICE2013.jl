...
function getdice2013gamsparameters(filename)
    ...
    p[:cost1]       = getparams(f, "B46:BI46", :all, sheet, T)         #Abatement cost function coefficient
    ...
    p[:elasmu]      = getparams(f, "B54:BI54", :single, sheet, 1)      #Elasticity of MU of consumption
    ...
    p[:MIU]         = getparams(f, "B47:BI47", :all, sheet, T)         #Optimized emission control rate results from DICE2013 (base case)
    ...
    p[:partfract]   = getparams(f, "B49:BI49", :all, sheet, T)         #Fraction of emissions in control regime
    p[:pbacktime]   = getparams(f, "B50:BI50", :all, sheet, T)         #Backstop price ($1000 per ton CO2)
    p[:rr]          = getparams(f, "B55:BI55", :all, sheet, T)         #Social Time Preference Factor
    p[:S]           = getparams(f, "B51:BI51", :all, sheet, T)         #Optimized savings rate (fraction of gross output) results from DICE2013 (base case)
    ...
    p[:tatm0]       = getparams(f, "B34:B34", :single, sheet, 1)       #Initial atmospheric temp change 2008-2011 (C from 1900)
    p[:tocean0]     = getparams(f, "B35:B35", :single, sheet, 1)       #Initial temperature of deep oceans (deg C aboce 1900)
    p[:usedamadj]   = false
